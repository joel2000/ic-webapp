pipeline {
     environment {
       ...
       STG_APP_ENDPOINT = "ip10-0-0-3-cen1q8gmjkegg872cd0g-8081.direct.docker.labs.eazytraining.fr"
       PROD_API_ENDPOINT = "ip10-0-0-3-cen1q8gmjkegg872cd0g-1993.direct.docker.labs.eazytraining.fr"
       PROD_APP_ENDPOINT = "ip10-0-0-3-cen1q8gmjkegg872cd0g-80.direct.docker.labs.eazytraining.fr"
       INTERNAL_PORT = "8080"
     ...
     stages {
      ...
      stage('Save Artefact') {
          ...
          steps {
             script {
               sh '''
                 docker save  ${ID_DOCKER}/$IMAGE_NAME:$IMAGE_TAG > ic-webapp.tar                 
               '''
             }
          }
     }          
     ...
     stage('STAGING - Deploy app') {
      ...
      steps {
          script {
            sh '''
              echo  {\\"your_name\\":\\"${APP_NAME}\\",\\"container_image\\":\\"${CONTAINER_IMAGE}\\", \\"external_port\\":\\"${EXTERNAL_PORT}81\\", \\"internal_port\\":\\"${INTERNAL_PORT}\\"}  > data.json 
